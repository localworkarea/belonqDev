(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".merch__slider")) {
            const marchSlider = new Swiper(".merch__slider", {
                modules: [ EffectCube, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            const navButtons2 = document.querySelectorAll(".nav-merch__nav-btn");
            navButtons2.forEach(((btn, index) => {
                btn.addEventListener("click", (() => {
                    marchSlider.slideToLoop(index);
                }));
            }));
            marchSlider.on("slideChange", (() => {
                const realIndex = marchSlider.realIndex;
                navButtons2.forEach(((btn, index) => {
                    if (index === realIndex) btn.classList.add("_active"); else btn.classList.remove("_active");
                }));
            }));
            navButtons2[marchSlider.realIndex].classList.add("_active");
        }
    }
